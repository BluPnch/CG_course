...
{
    public class SunflowerField : ISunObserver
    {
        private double _width;
        private double _height;
        private int _countSunflowers;
        private double _diffuseReflectance; 
        private double _specularReflectance;
        
        public Model3DGroup _modelGroup;
        private Brush _matcapTexture;
        ...
        private Dictionary<GeometryModel3D, Point> _shadowPositions = new Dictionary<GeometryModel3D, Point>();
        ...
        public SunflowerField(double width, double height, string pointsFilePath, int countSunflowers, string sunflowerModelPath, string sunflowerTexturePath)
        {
            ...
            CreateField();
            if (_countSunflowers > 0)
                AddSunflowers(); 
        }
        
        public void UpdateSunPosition(Vector3D sunPosition)
        {
            foreach (var shadowModel in _shadowModels)
            {
                UpdateShadowPosition(shadowModel, sunPosition);
            }
            OnShadowsUpdated();
        }

        private void UpdateShadowPosition(Model3D shadowModel, Vector3D sunDirection)
        {
            if (shadowModel is Model3DGroup shadowGroup)
            {
                foreach (var child in shadowGroup.Children)
                {
                    if (child is GeometryModel3D geometryModel)
                    {
                        var transform = geometryModel.Transform as Transform3DGroup;
                        if (transform == null)
                        {
                        ...
                        if (_shadowPositions.TryGetValue(geometryModel, out Point position))
                        {
                            double x = position.X;
                            double z = position.Y;

                            double stretchFactor = sunDirection.Z;
                            double coef_tension = 200.0;
                            stretchFactor = Clamp(stretchFactor, -coef_tension, coef_tension);

                            var scaleTransform = new ScaleTransform3D(1, 1, stretchFactor);
                            transform.Children.Add(scaleTransform);

                            var translateTransform = new TranslateTransform3D(x, 0.1, z);
        ...
        public void SetReflectanceCoefficients(double diffuse, double specular)
        {
            _diffuseReflectance = diffuse;
        ...
        private void UpdateFieldMaterials()
        {
            var plane = _modelGroup.Children.OfType<GeometryModel3D>().FirstOrDefault();
            if (plane != null)
            {
                var materialGroup = new MaterialGroup();
                var diffuseMaterial = new DiffuseMaterial(new SolidColorBrush(Color.FromArgb(255, 100, 255, 100)))
                {
                    Brush = new SolidColorBrush(Colors.Green) { Opacity = _diffuseReflectance }
                };
                ...
                {
                    Brush = new SolidColorBrush(Colors.White) { Opacity = _specularReflectance },
                    SpecularPower = 100
                };
                materialGroup.Children.Add(specularMaterial);

                
                plane.Material = materialGroup;
                plane.BackMaterial = materialGroup;
            }
        }
        ...
        private Model3D CreateSunflowerShadow(Model3DGroup sunflowerModelGroup, double x, double z)
        {
            ...
            foreach (var child in sunflowerModelGroup.Children)
            {
                if (child is GeometryModel3D geometryModel)
                {
                    ...
                    var shadowMaterial = new DiffuseMaterial(new SolidColorBrush(Color.FromArgb(128, 0, 0, 0)));

                    var shadowModel = new GeometryModel3D(flattenedGeometry, shadowMaterial);

                    _shadowPositions[shadowModel] = new Point(x, z);

                    var transformGroup = new Transform3DGroup();
                    transformGroup.Children.Add(new TranslateTransform3D(x, 0.1, z)); // Позиция тени чуть выше поверхности
                    shadowModel.Transform = transformGroup;
        ...
        private Model3DGroup LoadSunflowerModel(double x, double z)
        {
            ...
            _matcapTexture = new ImageBrush(new BitmapImage(new Uri(_sunflowerTexturePath)));
            ...
            foreach (var mesh in scene.Meshes)
            {
                ...
                Brush materialBrush = _matcapTexture;
